import { createTableRelationsHelpers, is, type Relation, Relations, type Table } from "drizzle-orm";
...
import { GraphQLError, type GraphQLInputObjectType, GraphQLInt, GraphQLList, GraphQLNonNull, type GraphQLObjectType } from "graphql";
...
const generateSelectArray = (
  ...
  const queryBase = db.query[tableName as keyof typeof db.query] as unknown as RelationalQueryBuilder<any, any, any> | undefined;
  if (!queryBase) {
    throw new Error(
      `Drizzle-GraphQL Error: Table ${tableName} not found in drizzle instance. Did you forget to pass schema to drizzle constructor?`,
    );
    ...
    resolver: async (_source, args: Partial<TableSelectArgs>, _context, info) => {
      try {
        ...
        const query = queryBase.findMany({
          ...
          with: relationMap[tableName] ? extractRelationsParams(relationMap, tables, tableName, parsedInfo, typeName) : undefined,
        });
  ...
  return {
    ...
    resolver: async (_source, args: { values: Record<string, any>[] }, _context, info) => {
      try {
        ...
        // biome-ignore lint/style/noNonNullAssertion: prevent ts error
        const columns = extractSelectedColumnsFromTreeSQLFormat<PgColumn>(parsedInfo.fieldsByTypeName[typeName]!, table);
...
export const generateSchemaData = <TDrizzleInstance extends PgDatabase<any, any, any>, TSchema extends Record<string, Table | unknown>>(
  db: TDrizzleInstance,
  schema: TSchema,
  ...
  const rawRelations = schemaEntries
    ...
    .map<[string, Relations]>(([_key, value]) => [
      // biome-ignore lint/style/noNonNullAssertion: prevent ts error
      tableEntries.find(([_tableName, tableValue]) => tableValue === (value as Relations).table)![0] as string,
      value as Relations,
    ])
      ...
      const namedConfig: Record<string, TableNamedRelations> = Object.fromEntries(
        Object.entries(config).map(([innerRelName, innerRelValue]) => [
          innerRelName,
          {
            ...
            // biome-ignore lint/style/noNonNullAssertion: prevent ts error
            targetTableName: tableEntries.find(([_tableName, tableValue]) => tableValue === innerRelValue.referencedTable)![0],
          },
        ]),
      );
