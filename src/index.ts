...
export const buildSchema = <TDbClient extends AnyDrizzleDB<any>>(db: TDbClient, config?: BuildSchemaConfig): GeneratedData<TDbClient> => {
  ...
  if (typeof config?.relationsDepthLimit === "number") {
    if (config.relationsDepthLimit < 0) {
      throw new Error("Drizzle-GraphQL Error: config.relationsDepthLimit is supposed to be nonnegative integer or undefined!");
    }
  ...
  // biome-ignore lint/suspicious/noImplicitAnyLet: satisfy ts definition for now
  let generatorOutput;
